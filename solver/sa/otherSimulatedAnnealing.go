...
func (s *OsaATSPSolver) SetGraph(g graph.Graph) {
	s.graph = g
}

// GetGraph zwraca przypisany graf
func (s *OsaATSPSolver) GetGraph() graph.Graph {
	return s.graph
}

// SetStartVertex ustawia wierzchołek startowy
func (s *OsaATSPSolver) SetStartVertex(startVertex int) {
	s.startVertex = startVertex
}

// SetTimeout ustawia czas wykonania w sekundach
func (s *OsaATSPSolver) SetTimeout(timeout int64) {
	s.timeout = timeout
}

// GetTimeout zwraca czas wykonania w sekundach
func (s *OsaATSPSolver) GetTimeout() int64 {
	return s.timeout
}

// NewSimulatedAnnealingATSPSolver tworzy nowy solver
func NewOtherSimulatedAnnealingATSPSolver(initialTemperature float64, minimalTemperature float64, alpha float64, iterations int, timeout int64) OsaATSPSolver {
	return OsaATSPSolver{
...
func (s *OsaATSPSolver) calculateCost(path []int) int {
...
func (s *OsaATSPSolver) getNeighbor(currentPath []int) []int {
...
func (s *OsaATSPSolver) acceptanceProbability(delta int, temperature float64) float64 {
	return math.Exp(-float64(delta) / temperature)
}

// Solve rozwiązuje ATSP metodą Symulowanego Wyżarzania
func (s *OsaATSPSolver) Solve() ([]int, int) {
